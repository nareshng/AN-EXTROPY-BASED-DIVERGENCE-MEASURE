...
##   * squared difference  : ggplot2::geom_step + dashed line at tau
##   * cumulative divergence: ggplot2::geom_step + dashed line at tau
##   * combined figure     : facet_wrap(~ Dataset, scales = "free")
...
##        figure directory, and the geom for the cumulative panel.
## Output: the three file paths invisibly; writes the three per-dataset PDFs.
fig_dataset_figures <- function(row, t1, s1, t2, s2, step_df, labels, fig_dir,
  ...
  base_name <- fig_clean_filename(paste(row$Dataset, labels[1L], "vs", labels[2L]))
  km_file <- file.path(fig_dir, paste0(base_name, "_KM_curve.pdf"))
  diff_file <- file.path(fig_dir, paste0(base_name, "_squared_difference.pdf"))
  cum_file <- file.path(fig_dir, paste0(base_name, "_cumulative_divergence.pdf"))
  ...
  ## ---- squared difference ------------------------------------------------------
  diff_plot <- ggplot2::ggplot(step_df, ggplot2::aes(x = left_time, y = diff_squared)) +
    ggplot2::geom_step(linewidth = 0.8) +
    ggplot2::geom_vline(xintercept = tau, linetype = "dashed") +
    ggplot2::labs(
      title = paste0(row$Dataset, ": squared survival difference"),
      subtitle = subtitle, x = "Time",
      y = expression((widehat(bar(F))[n[1]](t) - widehat(bar(G))[n[2]](t))^2)
    ) +
    ggplot2::theme_bw()
  ggplot2::ggsave(diff_file, diff_plot, width = 7.2, height = 4.8, device = "pdf")
  ...
  invisible(c(km = km_file, squared_difference = diff_file, cumulative = cum_file))
}
...
## Input: the stacked step data frames, the figure directory and the geom.
## Output: the file path invisibly; writes the combined faceted figure.
fig_combined_figure <- function(all_step_dfs, fig_dir, cumulative_geom = c("step", "line")) {
  cumulative_geom <- match.arg(cumulative_geom)
  layer <- if (cumulative_geom == "step") {
    ggplot2::geom_step(linewidth = 0.8)
  } else {
    ggplot2::geom_line(linewidth = 0.8)
  }
  combined <- ggplot2::ggplot(all_step_dfs,
    ...
    ggplot2::theme_bw()
  path <- file.path(fig_dir, "three_cases_combined_cumulative_divergence.pdf")
  ggplot2::ggsave(path, combined, width = 10, height = 5.8, device = "pdf")
  invisible(path)
}
