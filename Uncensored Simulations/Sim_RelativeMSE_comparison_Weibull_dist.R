...
WEIBULL_TAIL_PROBABILITY <- 1e-12
...
simulate_one_cell <- function(
  ...
  # Retain the original deterministic upper limit used for Table 2.
  q_upper <- max(
    qweibull(
      1 - WEIBULL_TAIL_PROBABILITY,
      shape = shape1,
      scale = scale1
    ),
    qweibull(
...
if (verbose) {
  cat(sprintf(
    paste0(
      "Table 2 configuration: B=%d, seed=%d, ",
